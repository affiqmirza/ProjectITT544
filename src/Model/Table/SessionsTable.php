...
class SessionsTable extends Table
{
    ...
    public function validationDefault(Validator $validator)
    {
        ...
        $validator
            ->scalar('problems')
            ->maxLength('problems', 250)
            ->requirePresence('problems', 'create')
            ->notEmpty('problems');

        $validator
            ->integer('Bookingid')
            ->requirePresence('Bookingid', 'create')
            ->notEmpty('Bookingid')
            ->add('Bookingid', 'unique', ['rule' => 'validateUnique', 'provider' => 'table']);
    ...
    {
        $rules->add($rules->isUnique(['Bookingid']));
